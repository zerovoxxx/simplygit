...
- **R13. ViewModel `launch` 块必须把异常收敛为 UiState，禁止逃出 `SupervisorJob`**：`viewModelScope.launch { val result = block() }` 中的 `block()` 若抛 Throwable，默认会被 `SupervisorJob` 静默丢进 `Thread.uncaughtExceptionHandler`，`_uiState` 永久停在 `Working` / `Loading` 等中间态，UI 被锁死。任何 `launch` 块里调用的 suspend 函数都必须**假定它可能违反领域契约抛异常**（如底层 `EncryptedSharedPreferences` 抛 `GeneralSecurityException`、Room 崩溃、Hilt 构造阶段异常），因此必须包 `try/catch`：保留 `CancellationException`（`throw ce`）保持结构化并发，其余 Throwable 统一转成 `Result.Failure` / `UiState.Error` 走既有的 sanitizer 管道。（来源：bug_report_20260503_p16x BUG-001，R8 的 ViewModel 入口侧补强）
- **R14. 后台中间态必须有租约与恢复路径**：凡是持久化的运行中状态（如 `syncState = RUNNING`、`sync_log.endedAt = null`）都不能只依赖正常结束路径收口；必须有启动前 stale recovery、取消时 `NonCancellable` 最小收口、以及"不覆盖更新摘要"的孤儿审计清理规则。WorkManager / 进程死亡 / 厂商后台清理都应被当作常态边界。（来源：BUG-20260531-B）
